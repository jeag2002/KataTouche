...
public final class Main {
  ...
  public void process(String[] args) {
    for (String arg : args) {
      if (arg.startsWith(INPUTS)) {
        engineBean.inputListeners = StringUtils
                .splitData(StringUtils.obtainDataFromParameter(arg, INPUTS));
      } else if (arg.startsWith(OUTPUTS)) {
        engineBean.outputListeners = StringUtils
                .splitData(StringUtils.obtainDataFromParameter(arg, OUTPUTS));
   ...
   * @throws InputBeanNotValidException inputBeanNotValidException
   * @throws InvalidPropertyFileException invalidPropertyFileException
   * @throws InvalidDateException  invalidDateException
   */
  public void init() throws InputBeanNotValidException, InvalidPropertyFileException, 
  ...
  public static void main(String[] args) throws 
    ...
    if (args.length < 4) {
      ...
      System.out.println("EXPECTED -inputs=i1,i2,... -outputs=o1,o2,... -property=props.properties "
            + "-date=YYYY/MM/DD (D=Day,M=Month,Y=Year)");
